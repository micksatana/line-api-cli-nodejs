...
describe('ThingsCommand', () => {
  ...
  describe('cli', () => {
    ...
    describe('when run with register:scenario-set operation', () => {
      beforeAll(() => {
        spyOn(ThingsCommand, 'getCommandLineArgs').mockReturnValue({
        ...
        spyOn(ThingsRegisterScenarioSetOperation, 'run').mockResolvedValue(undefined);
      });

      it('run correctly', async () => {
        await expect(ThingsCommand.cli()).resolves.toEqual(undefined);
        expect(ThingsRegisterScenarioSetOperation.run).toHaveBeenCalled();
      });

      afterAll(() => {
        ThingsCommand.getCommandLineArgs.mockRestore();
        ThingsRegisterScenarioSetOperation.run.mockRestore();
      });
    });

    describe('when run with get:scenario-set operation', () => {
      beforeAll(() => {
        spyOn(ThingsCommand, 'getCommandLineArgs').mockReturnValue({
        ...
        spyOn(ThingsGetScenarioSetOperation, 'run').mockResolvedValue(undefined);
      });

      it('run correctly', async () => {
        await expect(ThingsCommand.cli()).resolves.toEqual(undefined);
        expect(ThingsGetScenarioSetOperation.run).toHaveBeenCalled();
      });

      afterAll(() => {
        ThingsCommand.getCommandLineArgs.mockRestore();
        ThingsGetScenarioSetOperation.run.mockRestore();
      });
    });

    describe('when run with remove:scenario-set operation', () => {
      beforeAll(() => {
        spyOn(ThingsCommand, 'getCommandLineArgs').mockReturnValue({
          operation: 'remove:scenario-set',
          options: {},
          _unknown: []
        });
        spyOn(ThingsRemoveScenarioSetOperation, 'run').mockResolvedValue(undefined);
      });

      it('run correctly', async () => {
        await expect(ThingsCommand.cli()).resolves.toEqual(undefined);
        expect(ThingsRemoveScenarioSetOperation.run).toHaveBeenCalled();
      });

      afterAll(() => {
        ThingsCommand.getCommandLineArgs.mockRestore();
        ThingsRemoveScenarioSetOperation.run.mockRestore();
      });
    });
