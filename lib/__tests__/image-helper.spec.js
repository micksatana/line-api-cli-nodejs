...
describe('ImageHelper', () => {
  ...
  const chickHelpsPath = path.resolve(
    __dirname,
    '../../assets/chick-helps.png'
  );
  ...
  describe('when image is drawable', () => {
    ...
    it('draw image correctly', async () => {
      ...
      expect(terminal.drawImage.mock.calls[0][0]).toEqual(chickHelpsPath);
  ...
  describe('when image is NOT drawable', () => {
    beforeAll(() => {
      spyOn(terminal, 'drawImage').mockRejectedValueOnce(
        new Error('failed to draw')
      );
    });
