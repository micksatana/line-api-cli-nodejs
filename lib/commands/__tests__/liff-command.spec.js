...
describe('LIFFCommand', () => {
  ...
  describe('cli', () => {
    ...
    describe('when run with update operation', () => {
      beforeAll(() => {
        spyOn(LIFFCommand, 'getCommandLineArgs').mockReturnValue({
          operation: 'update',
          options: {},
        ...
        spyOn(LIFFUpdateOperation, 'run').mockResolvedValue(undefined);
      });

      it('run correctly', async () => {
        await expect(LIFFCommand.cli()).resolves.toEqual(undefined);
        expect(LIFFUpdateOperation.run).toHaveBeenCalled();
      });

      afterAll(() => {
        LIFFCommand.getCommandLineArgs.mockRestore();
        LIFFUpdateOperation.run.mockRestore();
      });
    });
