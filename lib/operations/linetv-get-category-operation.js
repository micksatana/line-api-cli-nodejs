...
export default class LINETvListCategoryOperation extends Operation {
  ...
  static async run() {
    ...
    if (!categories.length) {
      console.log('Category not found'.info);
      return true;
    }
      ...
      (await prompts(
        {
          ...
          validate: countPerPage => isNaN(countPerPage) || !countPerPage ? 'Please Enter Number' : true
        },
        this.cancelOption
      )) || {};
    ...
    const representClip = [getResponse.data.body.representClip];
    console.table(representClip.map(item => {
      const column = {};
      column['Represent Clip No.'.success] = item.clipNo;
      column['Represent Clip Title'.success] = item.clipTitle;
      column['Represent Clip URL'.success] = item.serviceUrl;
      column['Play Count'.success] = item.playCount;
      column['Likeit Count'.success] = item.likeitPoint;
      return column;
    }));
    console.table(
