...
import LINETvGetRankingRequest from '../linetv-ranking-request';
...
describe('LINERankingRequest', () => {
  describe('when create an instance with options.accessToken', () => {
    ...
    beforeAll(() => {
      ...
      req = new LINETvGetRankingRequest({ accessToken });
    });

    it('should have correct endpoint', () => {
      expect(req.endpoint).toEqual('https://api.line.me/line-tv/v1/ranking');
    });
    ...
    describe('when send a request', () => {
      ...
      it('should call to correct endpoint', () => {
        ...
        expect(req.axios.get).toHaveBeenCalledWith(
          `${req.endpoint}/clip?lineChannelId=${channelId}&country=${countryCode}&page=${page}&countPerPage=${countPerPage}`
        );
      });
