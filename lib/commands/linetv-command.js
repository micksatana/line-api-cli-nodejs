...
import LINETvListCurationOperation from '../operations/linetv-list-curation-operation';
  ...
  static async cli() {
    try {
      ...
      if (options.help) {
        ...
        switch (operation) {
          case 'list:curation':
            console.log(commandLineUsage(LINETvListCurationOperation.usage));
            break;

          default:
            console.log(
              commandLineUsage([
                ...LINETvListCurationOperation.usage,
              ])
            );
        }
        process.exit(0);
      ...
      if (operation === 'list:curation') {
        await LINETvListCurationOperation.run();
      } else {
        await ImageHelper.draw('chick-helps');
        console.log(
